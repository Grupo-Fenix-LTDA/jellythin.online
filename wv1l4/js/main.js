...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /*  5. Pricing Card Flip                                               */
  /* ------------------------------------------------------------------ */
  var flipBtns = document.querySelectorAll('.pricing-flip-btn');
  for (var fb = 0; fb < flipBtns.length; fb++) {
    flipBtns[fb].addEventListener('click', (function (btn) {
      return function () {
        var container = btn.closest('.pricing-flip-container');
        if (!container) return;

        var nowFlipped = container.classList.toggle('is-flipped');

        var label = btn.querySelector('.pricing-flip-btn-text');
        if (label) {
          label.textContent = nowFlipped ? 'View Special Offer' : 'One-Time Offer';
        }
        btn.setAttribute('aria-pressed', nowFlipped ? 'true' : 'false');
      };
    })(flipBtns[fb]));
  }
});
